...
// API lấy chữ cái ngẫu nhiên cho game
router.get('/random/:type', (req, res) => {
    const type = req.params.type; // 'hiragana' hoặc 'katakana'
    const data = type === 'hiragana' ? hiragana : katakana;
    
    // Gộp tất cả các loại chữ cái
    const allChars = [...data.basic, ...data.tenten, ...data.maru];
    
    // Lấy ngẫu nhiên một chữ cái
